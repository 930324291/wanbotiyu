...
public class HomeSupportFragment extends BaseFragment<HomeSupportPresenterImpl, HomeSupportModelImpl>
    ...
    private LinearLayout ll_home_zhudui,ll_home_shijiebo,ll_home_jingcai,ll_home_bifen,ll_home_chuqiang;
    ...
    private void initHeadView() {
        ...
        ll_home_zhudui = headView.findViewById(R.id.ll_home_zhudui);
        ll_home_shijiebo = headView.findViewById(R.id.ll_home_shijiebo);
        ll_home_jingcai = headView.findViewById(R.id.ll_home_jingcai);
        ll_home_bifen = headView.findViewById(R.id.ll_home_bifen);
        ll_home_chuqiang = headView.findViewById(R.id.ll_home_chuqiang);
        adapter.addHeaderView(headView);

        ll_home_zhudui.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), HomeDetailActivity.class);
                intent.putExtra("title", "主队");
                getActivity().startActivity(intent);
            }
        });
        ll_home_shijiebo.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), HomeDetailActivity.class);
                intent.putExtra("title", "世界播");
                getActivity().startActivity(intent);
            }
        });
        ll_home_jingcai.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), MWeb.class);
                intent.putExtra("url", "https://m.allwin368.com/Index/peking.html?bmapp=1&extratype=2");
                getActivity().startActivity(intent);
            }
        });
        ll_home_bifen.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), HomeDetailActivity.class);
                intent.putExtra("title", "比分");
                getActivity().startActivity(intent);
            }
        });
        ll_home_chuqiang.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), HomeDetailActivity.class);
                intent.putExtra("title", "出墙");
                getActivity().startActivity(intent);
            }
        });
    }
    ...
    //去除圈子、直播和广告
    private List<HomeDataBean.DataBean.PostsBeanX> handleList(List<HomeDataBean.DataBean.PostsBeanX> articles) {
        List<HomeDataBean.DataBean.PostsBeanX> s = new ArrayList<>();
        for(int i = 0;i<articles.size();i++){
            HomeDataBean.DataBean.PostsBeanX postsBeanX = articles.get(i);
            if(!TextUtils.isEmpty(postsBeanX.sourceUrl) && postsBeanX.sourceUrl.contains("http")){
                s.add(postsBeanX);
            }
        }
        return s;
...
//                posts.addAll(handleList(homeDataBean.data.posts));
